...
static void wifi_event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    if (s_retry_num < MAX_FAILURES) {
      ...
      xEventGroupSetBits(wifi_event_group, WIFI_FAILURE); // set value
    }
  }
}
...
esp_err_t connect_wifi() {
  ...
  // Check 3:46 for wi-fi programming model
  // "checking for events in the background"
  ...
  // create event group to see if event was successul or not
  wifi_event_group = xEventGroupCreate();

  esp_event_handler_instance_t wifi_handler_event_instance;
  ESP_ERROR_CHECK(esp_event_handler_instance_register(
      WIFI_EVENT,          // checking for errors in the wifi event
      ESP_EVENT_ANY_ID,    // any type of errors related to wifi ->
                           // !TODO - Change this later to wifi connect or
                           // disconnect from station... use as a filter
      &wifi_event_handler, //
      NULL,                //
      &wifi_handler_event_instance // instance id of event handler
                                   // later to be used to close the event
                                   // handler and unregister from the event loop
      ));
  ...
  EventBits_t bits = xEventGroupWaitBits(
      wifi_event_group,
      WIFI_SUCCESS | WIFI_FAILURE, // one of these must be true
...
void app_main(void) {
  ...
  // initialize storage for the wifi config
